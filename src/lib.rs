...
pub struct TcsData {
	...
	// 数据盘配置
	pub data_disks: Vec<TcsDisk>,
	// 网络配置
	pub internet_accessible: TcsInternet,
}
...
// DNS 解析记录
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct TcsInternet {
	#[serde(rename = "InternetChargeType")]
	pub internet_charge_type: String,
	#[serde(rename = "InternetMaxBandwidthOut")]
	pub internet_max_bandwidth_out: u8,
	#[serde(rename = "PublicIpAssigned")]
	pub public_ip_assigned: bool,
}
...
//                },
	...
	pub fn tcs_run_instances(&mut self, tcs_data: &TcsData) -> Result<String, Box<dyn Error>> {
		...
		// 网络配置
		let internet_accessible = tcs_data.internet_accessible.clone();
		...
		if !data_disks.is_empty() {
			payload["DataDisks"] = to_value(data_disks)?;
		}
